...
public class User {
    ...
    */
    public Integer validatePassword(String password) {
        ...
        //Check if password contains one special character. If yes add 1 to validity
        if( password.matches("(?=.*[~!@#$%^&*()_-]).*") )
            validity = validity + 1;

        if (validity >=2 ) {
            //Check if password contains one lower case letter. If yes add 1 to validity
            if( password.matches("(?=.*[a-z]).*") )
                validity = validity + 1;

            //Check if password contains one upper case letter. If yes add 1 to validity
            if( password.matches("(?=.*[A-Z]).*") )
                validity = validity + 1;
        }
        return validity;
